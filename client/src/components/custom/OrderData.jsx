...
export const OrderData = ({
  ...
  const handleDownloadInvoice = async () => {
    try {
      ...
      page.drawText("MS Store", {
        x: 400,
        y: 750,
        size: 12,
        color: rgb(1, 1, 1),
      });
      page.drawText("Shahjahanpur, Rajasthan", { x: 400, y: 735, size: 10 });
      page.drawText("Email: ms@company.com", { x: 400, y: 705, size: 10 });
    ...
    <Card className="grid gap-2 p-2">
      {
        products.map((product) => (
          <div key={product._id} className="flex flex-col sm:flex-row justify-between items-end sm:items-center border p-3 rounded-lg bg-gray-100 dark:bg-zinc-900">
        <div className="flex items-center gap-2">
          <img src={product?.id?.images[0]?.url} className="w-20 h-20 rounded-lg " alt={product?.id?.name} />
          <div className="grid gap-1">
            ...
            <p className="flex text-xs sm:text-md gap-2 sm:gap-2 text-gray-500 my-2 sm:my-0">
              <span>
                Color:{" "}
                <span style={{ backgroundColor: product?.color }}>{product?.color}</span>
              </span>
              <span>|</span>
        ...
        <div className="flex sm:flex-col gap-3 sm:gap-0 mt-2 sm:mt-0 sm:items-center">
          ...
          <p className="dark:text-customYellow text-end ">Qty: {product?.quantity}</p>
        </div>
      </div>
        ))
      }
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center">
        <span>Ordered On: <span className="capitalize">{new Date(createdAt).toLocaleString()}</span></span>
        <span onClick={handleDownloadInvoice} className="hover:underline text-sm cursor-pointer flex items-center gap-1 dark:text-customYellow">
            <ArrowDownToLine size={10}/>
            Download Invoice
        </span>
      </div>
      <hr />
      <span>Delivery At: <span className="capitalize">{address}</span></span>
    </Card>
  );
};
